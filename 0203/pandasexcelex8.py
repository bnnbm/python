...
import os #시스템 관련 모듈
input_path="D:\20190812\sql\python\pythontest/excel"
output_file="pandas_output8.xlsx"
#input_path,"*.xls*" : input_path 폴더의 모든 xlsx 또는 xls 파일 모
#all_workbooks : input_path폴더의 모든 excel 파일을 저장
all_workbooks = glob.glob(os.path.join(input_path,"*.xls*"))
data_frames = []
for workbook in all_workbooks :
    #workbook 한개의 엑셀파일 저장
    all_worksheets = pd.read_exce\
                  (workbook,sheet_name=None,index_col=None)
    for worksheet_name,data in all_worksheets.items() :
        data_frames.append(data)
#all_data_concat : 모든 엑셀파일의 data를 저장
all_data_concat = pd.concat\
      (data_frames,sort=False,axis=0,ignore_index=True)
